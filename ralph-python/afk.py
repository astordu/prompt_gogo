#!/usr/bin/env python3
"""Ralph 多轮迭代脚本（Python 版本，对应 ralph/afk.sh）。

用法: python afk.py <qodercli|claude|codex> [max_iterations]
"""
...
def usage() -> None:
    print(f"Usage: {sys.argv[0]} <qodercli|claude|codex> [max_iterations]", file=sys.stderr)
...
def main() -> int:
    ...
    max_iterations = sys.argv[2] if len(sys.argv) > 2 else "10"

    if adapter not in ("qodercli", "claude", "codex"):
        usage()
        return 1

    if re.fullmatch(r"[1-9][0-9]*", max_iterations) is None:
        print("max_iterations 必须是正整数。", file=sys.stderr)
        usage()
        return 1
    max_iterations = int(max_iterations)

    for i in range(1, max_iterations + 1):
        print(f"=== Ralph iteration {i}/{max_iterations} ({adapter}) ===")
        ...
        if issues == "[]":
            print(f"Ralph complete after {i - 1} iterations.")
            return 0
    ...
    if fetch_issues_with_label("ready-for-agent") == "[]":
        print(f"Ralph complete after {max_iterations} iterations.")
        return 0

    print(f"Ralph 停止，已完成 {max_iterations} 次迭代（达到上限）。")
    return 0
